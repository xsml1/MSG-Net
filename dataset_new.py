...
# from CAPTNet.utils.degradation_utils import Degradation
...
class PromptTrainDataset(Dataset):
    def __init__(self, args):
        ...
        self.de_dict = {'denoise': 0, 'dehazing': 1, 'deraining': 2, 'low-light': 3, 'underwater': 4}
    ...
    def _init_ids(self):
        if 'denoise' in self.de_type:
            self._init_denoise_ids()
        if 'dehazing' in self.de_type:
            self._init_dehazing_ids()
        if 'deraining' in self.de_type:
            self._init_deraning_ids()
        if 'low-light' in self.de_type:
            self._init_lowlight_ids()
        if 'underwater' in self.de_type:
            self._init_underwater_ids()

        random.shuffle(self.de_type)

    def _init_denoise_ids(self):
        temp_ids = []
        denoise_path = os.path.join(self.args.train_file_dir, 'denoise', 'images')
        denoise_files = os.listdir(denoise_path)
        temp_ids = [os.path.join(denoise_path, file) for file in denoise_files]
        self.denoise_ids = [{"clean_id" : x, "de_type" : 0} for x in temp_ids]

        self.denoise_counter = 0
        self.num_noise = len(self.denoise_ids)
        print("Total Denoise Ids : {}".format(self.num_noise))

    def _init_dehazing_ids(self):
        temp_ids = []
        dehazing_path = os.path.join(self.args.train_file_dir, 'dehazing', 'images')
        dehazing_files = os.listdir(dehazing_path)
        temp_ids = [os.path.join(dehazing_path, file) for file in dehazing_files]
        self.dehazing_ids = [{"clean_id" : x, "de_type" : 1} for x in temp_ids]

        self.dehazing_counter = 0
        self.num_hazing = len(self.dehazing_ids)
        print("Total Hazy Ids : {}".format(self.num_hazing))

    def _init_deraning_ids(self):
        temp_ids = []
        deraining_path = os.path.join(self.args.train_file_dir, 'deraining', 'images')
        deraining_files = os.listdir(deraining_path)
        temp_ids = [os.path.join(deraining_path, file) for file in deraining_files]
        self.deraining_ids = [{"clean_id" : x, "de_type" : 2} for x in temp_ids]

        self.deraining_counter = 0
        self.num_raining = len(self.deraining_ids)
        print("Total Rainy Ids : {}".format(self.num_raining))

    def _init_lowlight_ids(self):
        temp_ids = []
        lowlight_path = os.path.join(self.args.train_file_dir, 'low-light', 'images')
        lowlight_files = os.listdir(lowlight_path)
        temp_ids = [os.path.join(lowlight_path, file) for file in lowlight_files]
        self.lowlight_ids = [{"clean_id" : x, "de_type" : 3} for x in temp_ids]

        self.lowlight_counter = 0
        self.num_lowlight = len(self.lowlight_ids)
        print("Total Low-light Ids : {}".format(self.num_lowlight))

    def _init_underwater_ids(self):
        temp_ids = []
        underwater_path = os.path.join(self.args.train_file_dir, 'underwater', 'images')
        underwater_files = os.listdir(underwater_path)
        temp_ids = [os.path.join(underwater_path, file) for file in underwater_files]
        self.underwater_ids = [{"clean_id" : x, "de_type" : 4} for x in temp_ids]

        self.underwater_counter = 0
        self.num_underwater = len(self.underwater_ids)
        print("Total Underwater Ids : {}".format(self.num_underwater))
    ...
    def _merge_ids(self):
        ...
        if "deraining" in self.de_type:
            self.sample_ids+= self.deraining_ids
        if "dehazing" in self.de_type:
            self.sample_ids+= self.dehazing_ids
        if "low-light" in self.de_type:
            self.sample_ids+= self.lowlight_ids
        if "underwater" in self.de_type:
            self.sample_ids+= self.underwater_ids
        print(len(self.sample_ids))
    ...
    def __getitem__(self, idx):
        ...
        if de_id == 0:
            ...
            # self.denoise_counter = (self.denoise_counter + 1) % self.num_noise
            # if self.denoise_counter == 0:
            #     random.shuffle(self.denoise_ids)
        elif de_id == 1:
            # Dehazing
            degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
            clean_name = self._get_gt_name(sample["clean_id"])
            ...
            # self.dehazing_counter = (self.dehazing_counter + 1) % self.num_hazing
            # if self.dehazing_counter == 0:
            #     random.shuffle(self.dehazing_ids)
        elif de_id == 2:
            # Deraining
            degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
            clean_name = self._get_gt_name(sample["clean_id"])
            ...
            # self.deraining_counter = (self.deraining_counter + 1) % self.num_raining
            # if self.deraining_counter == 0:
            #     random.shuffle(self.deraining_ids)
        elif de_id == 3:
            # Low-light
            degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
            clean_name = self._get_gt_name(sample["clean_id"])
            ...
            # self.underwater_counter = (self.underwater_counter + 1) % self.num_underwater
            # if self.underwater_counter == 0:
            #     random.shuffle(self.underwater_ids)
...
def get_eval_set(dataset_path, tasks, image_size=256):
    ...
    for task in tasks:
        ...
        data_list = [os.path.join(data_dir, x) for x in os.listdir(data_dir) if is_image_file(x)]
...
class DatasetFromFolderEval(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = Image.open(label_filenames).convert('RGB')
        _, file = os.path.split(self.data_filenames[index])
...
class TestSpecificDataset(Dataset):
    def __init__(self, args):
        ...
        self.args = args
        self.degraded_ids = []
        self._init_clean_ids(args.val_file_dir)
...
class TestAllDataset(Dataset):
    def __init__(self, subset_path, image_size=256):
        super(TestAllDataset, self).__init__()
        self.image_size = image_size
        self.images_paths = []
        self.labels_paths = []
        for sub_path in subset_path:
            image_folder = os.path.join(sub_path, "images")
            label_folder = os.path.join(sub_path, "labels")
            image_files = [os.path.join(image_folder, f) for f in os.listdir(image_folder) if
                           os.path.isfile(os.path.join(image_folder, f))]
            label_files = [os.path.join(label_folder, f) for f in os.listdir(label_folder) if
                           os.path.isfile(os.path.join(label_folder, f))]
            image_files.sort()
            label_files.sort()
            self.images_paths.extend(image_files)
            self.labels_paths.extend(label_files)
        print("Total Images : {}".format(self.__len__()))
        # self._init_clean_ids(self.images_paths)
    ...
    def __getitem__(self, idx):
        # H,W,C = cv2.imread(self.labels_paths[idx]).shape
        # if(C != 3):
        #     print(idx,'---------------------',self.labels_paths[idx])
        # print(C,H,W,'\n')
        degraded_img = crop_img(np.array(Image.open(self.labels_paths[idx]).convert('RGB')), base=16)
        gt_img = crop_img(np.array(Image.open(self.images_paths[idx]).convert('RGB')), base=16)

        # name = self.degraded_ids[idx].split('/')[-1][:-4]
        # if (H != 256):
        #     degraded_img = degraded_img.resize((self.image_size, self.image_size))
        #     gt_img = gt_img.resize((self.image_size, self.image_size))
        degraded_img = self.toTensor(degraded_img)
...
subset_paths = ['/media/aa/a4b46d17-0f49-4392-98d6-49a5c9dee8e9/xujie/AIOIR/data/test/dehazing',
                '/media/aa/a4b46d17-0f49-4392-98d6-49a5c9dee8e9/xujie/AIOIR/data/test/denoise',
                '/media/aa/a4b46d17-0f49-4392-98d6-49a5c9dee8e9/xujie/AIOIR/data/test/deraining',
                '/media/aa/a4b46d17-0f49-4392-98d6-49a5c9dee8e9/xujie/AIOIR/data/test/low-light',
                '/media/aa/a4b46d17-0f49-4392-98d6-49a5c9dee8e9/xujie/AIOIR/data/test/underwater']

test_dataset = TestAllDataset(subset_paths)
# val_loader = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False, num_workers=16, drop_last=False, pin_memory=True)
# for ii, data_val in enumerate((val_loader), 0):
#     print(ii)
#     target = data_val[1]
#     input_ = data_val[0]
#     print(target.shape, type(target),type(input_))
#
